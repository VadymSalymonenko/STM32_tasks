...
int main(void)
{
  ...
  MyGPIO_Init(GPIOA, GPIO_PIN_0, MY_GPIO_INPUT, MY_GPIO_PULLUP, MY_GPIO_SPEED_LOW);
  MyGPIO_Init(GPIOA, GPIO_PIN_5, MY_GPIO_OUTPUT, MY_GPIO_NOPULL, MY_GPIO_SPEED_LOW);
  ...
  UART_Print("start\r\n");
    ...
    uint32_t frequency = 500; // частота в Гц
    uint32_t dutyCycle = 41; // 0-255
    char valueStr[20];

    int result_PWM = MyGPIO_PWM_Init(GPIOA, GPIO_PIN_8, TIM1, 0, frequency, dutyCycle);
